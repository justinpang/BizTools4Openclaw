"""business/customer_send/channels/wechat_channel — 企业微信推送。"""
...
logger = get_logger("customer_send.wechat")
...
class WechatChannel:
    ...
    def build_recipient(self, target: Any) -> str:
        # 企业微信 webhook 无 recipient 概念，这里用 opportunity_id 作为唯一标识
    ...
    def send(
        ...
        if not webhook_url or not webhook_url.startswith("http"):
            return False, 400, "wechat: invalid webhook url"

        # 若 content 是合法 JSON，视作完整消息体；否则包装为 markdown 文本
        try:
        ...
        try:
            ...
            if 200 <= resp.status_code < 300:
                ...
                return False, resp.status_code, f"wechat_api: {data.get('errmsg', resp.text[:200])}"
